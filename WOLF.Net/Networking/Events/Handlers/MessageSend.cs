        ...
        public override async void Handle(BaseMessage data)
        {
            ...
            try
            {
                ...
                if (secrets.Keys.Any((key) => Regex.IsMatch(fixedMessage.Content, $@"\A{key}\b")))
                {
                    ...
                    {
                        await Bot.Messaging().SendMessageAsync(fixedMessage.IsGroup ? fixedMessage.TargetGroupId : fixedMessage.SourceSubscriberId, string.Format(phrases.OrderBy((secret) => Guid.NewGuid()).FirstOrDefault(), Assembly.GetExecutingAssembly().GetName().Version), fixedMessage.MessageType);
                        return;
                    }
                }
