...
namespace WOLF.Net
{
    ...
    {
        internal List<Charm> charmCache  = new List<Charm>();
        ...
        {
            if (charmCache.Count > 0 && !requestNew)
                return charmCache;

            var result = await _webSocket.Emit<Response<List<Charm>>>(Request.CHARM_LIST);

            if (result.Success)
                charmCache = result.Body;
        ...
        public async Task<Response<CharmStatistics>> GetSubscriberStatisticsAsync(int subscriberId) => await _webSocket.Emit<Response<CharmStatistics>>(Request.CHARM_SUBSCRIBER_STATISTICS, new
        {
            id = subscriberId
        });
        ...
        public async Task<Response<List<CharmStatus>>> GetSubscriberActiveListAsync(int subscriberId, int offset = 0, int limit = 25) => await _webSocket.Emit<Response<List<CharmStatus>>>(Request.CHARM_SUBSCRIBER_ACTIVE_LIST, new
        {
            id = subscriberId,
            offset,
            limit
        ...
        /// <param name="limit"></param>
        /// <returns>Response<List<CharmStatus>></returns>
        public async Task<Response<List<CharmStatus>>> GetSubscriberExpiredListAsync(int subscriberId, int offset = 0, int limit = 25) => await _webSocket.Emit<Response<List<CharmStatus>>>(Request.CHARM_SUBSCRIBER_EXPIRED_LIST, new
        {
            id = subscriberId,
            offset,
            limit
        ...
        public async Task<Response<List<CharmSummary>>> GetSubscriberCharmSummaryAsync(int subscriberId) => await _webSocket.Emit<Response<List<CharmSummary>>>(Request.CHARM_SUBSCRIBER_SUMMARY_LIST, new
        {
            id = subscriberId,
        });
    }
