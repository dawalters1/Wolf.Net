...
namespace WOLF.Net
{
    public partial class WolfBot
    {
        ...
        public async Task<List<Achievement>> GetAchievementsAsync(Language language = Language.ENGLISH, bool requestNew = false)
        {
            ...
            var result = await _webSocket.Emit<Response<List<Achievement>>>(Request.ACHIEVEMENT_LIST, new
            {
        ...
        {
            return await _webSocket.Emit<Response<List<SubscriberAchievement>>>(Request.ACHIEVEMENT_SUBSCRIBER_LIST, new
            {
                id = subscriberId
            });
        }
