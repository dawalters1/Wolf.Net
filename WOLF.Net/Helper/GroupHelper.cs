...
namespace WOLF.Net.Helper
{
    public class GroupHelper : BaseHelper<Group>
    {
        ...
        /// <param name="password"></param>
        /// <returns>Response</returns>
        public async Task<Response> JoinAsync(string name, string password = null) => await WebSocket.Emit<Response>(Request.GROUP_MEMBER_ADD, new { name = name.ToLowerInvariant(), password });
        ...
        /// <returns>Response</returns>
        public async Task<Response> UpdateGroupSubscriberAsync(int groupId, int subscriberId, ActionType capability) => await WebSocket.Emit<Response>(Request.GROUP_MEMBER_UPDATE, new { groupId, id = subscriberId, capabilities = (int)capability });
