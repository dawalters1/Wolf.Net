    ...
    public class FormManager
    {
        ...
        private async Task<bool> FindForm(Message message)
        {
            ...
            if (_bot._usingTranslations)
            {
                ...
                if (forms.Count > 0)
                {
                    var phrase = _bot.Phrase().cache.Where(r => forms.Any(s => s.Value.Trigger.IsEqual(r.Name)) && message.Content.StartsWithCommand(r.Value)).OrderByDescending(r => r.Value.Length).FirstOrDefault();
                    ...
                    {
                        Group = message.IsGroup ? await _bot.Group().GetByIdAsync(message.TargetGroupId) : null,
                        Subscriber = await _bot.Subscriber().GetByIdAsync(message.SourceSubscriberId),
